...
def _build_writer(
    ...
    *,
    ...
    preprocessing_config.ensure_dirs()
    preprocessing_config.dry_run = dry_run
    preprocessing_config.backup_enabled = backup_enabled
    ...
    return writer, stats, preprocessing_config


def _minimal_writer(preprocessing_config: PreprocessingConfig) -> ExtractionWriter:
...
def _src_file(preprocessing_config: PreprocessingConfig, name: str, content: bytes) -> FileData:
    """Create a FileData backed by a real file in a temp source directory."""
    src = preprocessing_config.paths.source_dir / "src_files" / name
...
class TestExtractCompletePair:
    def test_writes_both_files_and_increments_stats(
        self, preprocessing_config: PreprocessingConfig
    ) -> None:
        writer, stats, config = _build_writer(preprocessing_config)
        pair = FilePair(
            stem="s",
            image=_src_file(preprocessing_config, "s.jpg", b"img"),
            annotation=_src_file(preprocessing_config, "s.txt", b"0 0.5 0.5 0.1 0.1\n"),
        )
    ...
    def test_raises_on_incomplete_pair(self, preprocessing_config: PreprocessingConfig) -> None:
        writer = _minimal_writer(preprocessing_config)
        pair = FilePair(stem="x", image=FileData(stem="x", name="x.jpg"))

        with pytest.raises(ValueError, match="Incomplete pair"):
            writer.extract_complete_pair(pair)

    def test_skips_when_both_destinations_exist(
        self, preprocessing_config: PreprocessingConfig
    ) -> None:
        writer, stats, config = _build_writer(preprocessing_config, skip_existing=True)
        (config.paths.labelized_images / "s.jpg").write_bytes(b"old")
        (config.paths.labelized_annotations / "s.txt").write_bytes(b"old")
        pair = FilePair(
            stem="s",
            image=_src_file(preprocessing_config, "s.jpg", b"new"),
            annotation=_src_file(preprocessing_config, "s.txt", b"new"),
        )
    ...
    def test_quarantines_when_image_invalid(
        self, preprocessing_config: PreprocessingConfig
    ) -> None:
        ...
        writer, stats, config = _build_writer(preprocessing_config, image_valid=False)
        pair = FilePair(
            stem="bad",
            image=_src_file(preprocessing_config, "bad.jpg", b"corrupt"),
            annotation=_src_file(preprocessing_config, "bad.txt", b"ann"),
        )
    ...
    def test_quarantines_when_yolo_annotation_invalid(
        self, preprocessing_config: PreprocessingConfig
    ) -> None:
        yolo = MagicMock(spec=YOLOValidator)
        yolo.validate.return_value = ValidationResult(is_valid=False, error="bad annotation")
        writer, stats, config = _build_writer(preprocessing_config, yolo_validator=yolo)
        pair = FilePair(
            stem="bad",
            image=_src_file(preprocessing_config, "bad.jpg", b"ok-img"),
            annotation=_src_file(preprocessing_config, "bad.txt", b"garbage"),
        )
    ...
    def test_skips_annotation_validation_when_no_yolo_validator(
        ...
        writer, stats, _ = _build_writer(preprocessing_config, yolo_validator=None)
        pair = FilePair(
            stem="s",
            image=_src_file(preprocessing_config, "s.jpg", b"img"),
            annotation=_src_file(preprocessing_config, "s.txt", b"anything-here"),
        )
    ...
    def test_dry_run_increments_stats_without_writes(
        self, preprocessing_config: PreprocessingConfig
    ) -> None:
        writer, stats, config = _build_writer(preprocessing_config, dry_run=True)
        pair = FilePair(
            stem="s",
            image=_src_file(preprocessing_config, "s.jpg", b"img"),
            annotation=_src_file(preprocessing_config, "s.txt", b"ann"),
        )
...
class TestExtractOrphans:
    def test_image_only_quarantines_and_counts(
        self, preprocessing_config: PreprocessingConfig
    ) -> None:
        writer, stats, config = _build_writer(preprocessing_config)
        pair = FilePair(stem="o", image=_src_file(preprocessing_config, "o.jpg", b"img"))
    ...
    def test_image_only_raises_when_no_image(self, preprocessing_config: PreprocessingConfig) -> None:
        with pytest.raises(ValueError, match="Missing image"):
            _minimal_writer(preprocessing_config).extract_image_only(FilePair(stem="x"))
    ...
    def test_image_only_skips_write_when_backup_disabled(
        ...
        writer, stats, config = _build_writer(preprocessing_config, backup_enabled=False)
        pair = FilePair(stem="o", image=_src_file(preprocessing_config, "o.jpg", b"img"))
    ...
    def test_annotation_only_quarantines_and_counts(
        self, preprocessing_config: PreprocessingConfig
    ) -> None:
        writer, stats, config = _build_writer(preprocessing_config)
        pair = FilePair(stem="o", annotation=_src_file(preprocessing_config, "o.txt", b"ann"))
    ...
    def test_annotation_only_raises_when_no_annotation(
        self, preprocessing_config: PreprocessingConfig
    ) -> None:
        with pytest.raises(ValueError, match="Missing annotation"):
            _minimal_writer(preprocessing_config).extract_annotation_only(FilePair(stem="x"))
...
class TestWriteUnlabeledImage:
    def test_writes_valid_image(self, preprocessing_config: PreprocessingConfig) -> None:
        writer, stats, config = _build_writer(preprocessing_config)
        img = _src_file(preprocessing_config, "u.jpg", b"uimg")
    ...
    def test_skips_when_destination_exists(
        self, preprocessing_config: PreprocessingConfig
    ) -> None:
        writer, stats, config = _build_writer(preprocessing_config, skip_existing=True)
        (config.paths.unlabeled / "u.jpg").write_bytes(b"old")
        img = _src_file(preprocessing_config, "u.jpg", b"new")
    ...
    def test_quarantines_invalid_image(self, preprocessing_config: PreprocessingConfig) -> None:
        writer, stats, config = _build_writer(preprocessing_config, image_valid=False)
        img = _src_file(preprocessing_config, "bad.jpg", b"corrupt")
    ...
    def test_hashes_name_on_collision(self, preprocessing_config: PreprocessingConfig) -> None:
        ...
        writer, stats, config = _build_writer(preprocessing_config, skip_existing=False)
        # Pre-create a file to force the name collision.
        (config.paths.unlabeled / "u.jpg").write_bytes(b"existing")
        img = _src_file(preprocessing_config, "u.jpg", b"new-content")
...
class TestWriteClassesFile:
    def test_writes_content(self, preprocessing_config: PreprocessingConfig) -> None:
        writer, _, config = _build_writer(preprocessing_config)
        writer.write_classes_file(b"cat\ndog\n")
        assert config.paths.classes_file.read_bytes() == b"cat\ndog\n"

    def test_dry_run_skips_write(self, preprocessing_config: PreprocessingConfig) -> None:
        writer, _, config = _build_writer(preprocessing_config, dry_run=True)
        writer.write_classes_file(b"cat\ndog\n")
...
class TestQuarantineDuplicates:
    def test_duplicate_image_increments_stat(self, preprocessing_config: PreprocessingConfig) -> None:
        writer, stats, _ = _build_writer(preprocessing_config)
        fd = _src_file(preprocessing_config, "dup.jpg", b"img")

        writer.quarantine_labelized_duplicate_image(fd)

        assert stats.duplicate_labelized_images == 1

    def test_duplicate_annotation_increments_stat(
        self, preprocessing_config: PreprocessingConfig
    ) -> None:
        writer, stats, _ = _build_writer(preprocessing_config)
        fd = _src_file(preprocessing_config, "dup.txt", b"ann")
...
def test_inc_rejects_unknown_field(preprocessing_config: PreprocessingConfig) -> None:
    with pytest.raises(AttributeError, match="no field"):
        _minimal_writer(preprocessing_config)._inc("nonexistent_counter")
